...
def toggle_article_subscription(request, pk):
    """
    Toggle reader subscription to a specific article.
    
    Subscribe if not subscribed, unsubscribe if already subscribed.
    """
    article = get_object_or_404(Article, pk=pk)
    subscription = ArticleSubscription.objects.filter(reader=request.user, article=article).first()

    if subscription:
        subscription.delete()
        messages.success(request, f"Unsubscribed from '{article.title}'.")
    else:
        ArticleSubscription.objects.create(reader=request.user, article=article)
...
def subscribe_publishers(request):
    """
    Manage reader subscription to publishers.
    """
    if request.method == "POST":
        form = PublisherSubscriptionForm(request.POST)
        if form.is_valid():
            request.user.reader_subscriptions.set(form.cleaned_data["publishers"])
            messages.success(request, "Publisher subscriptions updated.")
            return redirect("subscribe_publishers")
    else:
        form = PublisherSubscriptionForm(initial={"publishers": request.user.reader_subscriptions.all()})
...
def subscribe_journalists(request):
    """
    Manage reader subscription to journalists.
    """
    form = JournalistSubscriptionForm(request.POST or None)
    if request.method == "POST" and form.is_valid():
        request.user.journalist_subscriptions.set(form.cleaned_data["journalists"])
        selected_journalists = ", ".join(j.username for j in form.cleaned_data["journalists"])
        messages.success(request, f"Selected journalists: {selected_journalists or 'none'}.")
        return redirect("subscribe_journalists")

    if request.method != "POST":
        form = JournalistSubscriptionForm(
            initial={"journalists": request.user.journalist_subscriptions.all()}
        )
